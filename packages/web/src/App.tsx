...
interface ReviewComment { id: string; file: string; side: CommentSide; line: number; hunkHeader: string; selectedCode: string; comment: string; severity: CommentSeverity; status: CommentStatus; author?: 'human' | 'ai'; createdAt: string; updatedAt: string; }
...
export function App() {
  ...
  // AI code review: model reads the diff and adds anchored review comments.
  async function runAiReview() {
    setReviewing(true);
    try {
      const r = await api<{ created: number; total: number }>('/api/review/generate', { method: 'POST', body: '{}' });
      const cs = await api<ReviewComment[]>('/api/comments');
      setComments(cs);
      flash(r.created ? `AI review added ${r.created} comment${r.created > 1 ? 's' : ''}` : 'AI review found no issues 🎉');
      if (r.created) setActiveTab('comments');
    } catch (e) { setError(msgFor(e)); }
    ...
    <div className={cls}>
      <div className="comment-header">
        <div className="comment-header-left">
          {comment.author === 'ai' && <span className="ai-badge" title="Generated by AI review">AI</span>}
